...
from ..models.meeting import Meeting, MeetingAgendaItem, MeetingVote
...
def home():
    ...
    try:
        ...
        meetings = session.query(Meeting).order_by(Meeting.starts_at.desc()).limit(10).all()
        return render_template("admin.html", users=users, blocks=blocks, txs=txs, meetings=meetings)
    finally:
        session.close()
...
def add_finance():
    ...
    try:
        tx = Transaction(
            type=request.form.get("type", "income"),
        ...
        session.add(tx)
        session.commit()
        flash("Фінансову операцію додано.","success")
        return redirect(url_for("admin.home"))
    finally:
        session.close()
...
def create_meeting():
    ...
    try:
        meeting = Meeting(
            ...
            description=request.form.get("description", ""),
            starts_at=datetime.fromisoformat(request.form.get("starts_at", datetime.utcnow().isoformat()))
        )
        session.add(meeting)
        session.commit()
        flash("Зборы створено.","success")
        return redirect(url_for("admin.home"))
    finally:
        session.close()
...
def meetings():
    session = db_session()
    try:
        meetings = session.query(Meeting).order_by(Meeting.starts_at.desc()).all()
